...
class WebAudioManager {
     ...
     */
    async loadTrack(trackId, filePath, label) {
        ...
        try {
            ...
            if (arrayBuffer.byteLength > 50 * 1024 * 1024) { // > 50MB
                console.warn(`[WebAudioManager] Large audio file (${(arrayBuffer.byteLength / 1024 / 1024).toFixed(1)}MB) may cause UI freezing during decode`);
            }

            // Yield before decoding to prevent blocking
            await new Promise(resolve => setTimeout(resolve, 10));

            // Decode audio data - use a copy to avoid blocking
            let audioBuffer;
            try {
                console.log(`[WebAudioManager] Starting audio decode for track ${trackId}...`);

                // For large files, decodeAudioData blocks the main thread
                // Clone the buffer to avoid reference issues
                const bufferCopy = arrayBuffer.slice(0);

                // Yield before decode to let UI update
                await new Promise(resolve => setTimeout(resolve, 50));

                // Create a new promise that yields periodically
                audioBuffer = await new Promise((resolve, reject) => {
                    // Start the decode
                    this.audioContext.decodeAudioData(bufferCopy)
                        .then(buffer => {
                            console.log(`[WebAudioManager] Audio decode completed for track ${trackId}`);
                            resolve(buffer);
                        })
                        .catch(error => {
                            console.error(`[WebAudioManager] Decode error for track ${trackId}:`, error);
                            reject(error);
                        });

                    // Show a loading indicator after a short delay if still decoding
                    setTimeout(() => {
                        if (!audioBuffer) {
                            console.log(`[WebAudioManager] Still decoding track ${trackId}...`);
                        }
                    }, 1000);
                });

            } catch (decodeError) {
                console.error(`[WebAudioManager] Failed to decode audio for track ${trackId}:`, decodeError);
                // Try to provide more context about the error
                if (decodeError.message?.includes('Unable to decode')) {
                    throw new Error(`Audio format not supported or file corrupted: ${decodeError.message}`);
                }
                throw decodeError;
            }
