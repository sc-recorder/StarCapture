...
class YouTubeProvider extends EventEmitter {
    ...
    async upload(credentials, config, filePath, metadata = {}, onProgress) {
        ...
        // Note: This requires the YouTube account to have custom thumbnail permissions (verified account)
        if (videoId && metadata.thumbnailPath) {
            try {
                ...
                // 403 errors are common if the account doesn't have custom thumbnail permissions
                if (error.message.includes('403')) {
                    console.log('[YouTube] Cannot set custom thumbnail - YouTube account may need verification');
                    console.log('[YouTube] Visit https://www.youtube.com/features to check account status');
                } else {
                    console.error('[YouTube] Failed to upload thumbnail:', error.message);
                }
                // Don't fail the video upload if thumbnail upload fails
            }
        }
     ...
     * Requires youtube.upload scope AND YouTube account with custom thumbnail permissions
     */
    async uploadThumbnail(accessToken, videoId, thumbnailPath) {
        const fs = require('fs');
                ...
                res.on('end', () => {
                    if (res.statusCode === 200 || res.statusCode === 201) {
                        try {
                            ...
                            // Sometimes YouTube returns non-JSON on success
                            resolve({ success: true });
                        }
                    } else {
                        reject(new Error(`Failed to upload thumbnail: ${res.statusCode} - ${body}`));
                    }
                });
            });
