...
class OAuthPKCE {
    ...
    /**
     * Get YouTube channel info using access token
     * @param {string} accessToken - Access token
     * @returns {Promise<Object>} Channel information
     */
    async getGoogleChannelInfo(accessToken) {
        try {
            ...
            if (data.items && data.items.length > 0) {
                ...
                // Transform to a format compatible with the old userInfo structure
                return {
                    id: channel.id,
                    name: channel.snippet.title,
                    email: '', // No email available from YouTube API
                    picture: channel.snippet.thumbnails?.default?.url || '',
                    // Include additional YouTube-specific data
            ...
            console.error('[OAuth PKCE] Error getting YouTube channel info:', error);
            return null;
        }
    }
            ...
            if (reqUrl.pathname === url.parse(redirectUri).pathname) {
                ...
                if (this.pendingAuth && this.googleClient && this.googleCodeVerifier) {
                    try {
                        ...
                        // Get channel info
                        const userInfo = await this.getGoogleChannelInfo(tokens.access_token);

                        // Calculate expires_in and expires_at
                        ...
                        const result = {
                            ...
                            expires_at: expiresAt,  // Include expires_at timestamp
