...
class OBSCapabilityDetector {
    constructor() {
        this.obsPath = path.join(process.cwd(), 'resources', 'obs-studio', 'bin', '64bit', 'obs64.exe');
    ...
    async generateWebSocketConfig() {
        const obsRootPath = path.join(process.cwd(), 'resources', 'obs-studio');
...
`;
    ...
    async detectEncoders() {
        try {
            ...
            const logPath = path.join(process.cwd(), 'resources', 'obs-studio', 'config', 'obs-studio', 'logs');
            try {
