...
class WebSocketManager extends BaseManager {
    ...
    /**
     * Execute OBS command with timeout
     */
    async executeCommand(command, params = {}, timeoutMs = 5000) {
        if (!this.connected) {
            this.logger.error('Not connected to OBS');
        ...
        try {
            // Wrap OBS call with timeout
            const result = await this.callWithTimeout(
                this.obs.call(command, params),
                timeoutMs,
                `OBS command '${command}' timed out after ${timeoutMs}ms`
            );
     ...
     */
    async callWithTimeout(promise, timeoutMs, timeoutMessage) {
        let timeoutHandle;
    ...
    async stopRecording() {
        ...
        // StopRecord can take a long time (30+ seconds) for large files
        // as OBS finalizes the video file, flushes encoder buffers, writes indexes, etc.
        // Use a 60 second timeout to prevent indefinite hangs while allowing time to complete
        try {
            const result = await this.executeCommand('StopRecord', {}, 60000);
            ...
            // If timeout occurs, the RecordStateChanged event should still fire when OBS finishes
            this.logger.warn('StopRecord command timed out, but recording may still stop via event');
            throw error;
        }
    }
    ...
    async getRecordingStats() {
        ...
        try {
            // Get recording status
            const recordStatus = await this.obs.call('GetRecordStatus');
            // Removed debug logging - this runs every second and creates GB of logs
