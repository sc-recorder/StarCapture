...
class SCLogMonitor extends BaseManager {
    constructor() {
        ...
        this.eventCache = new Map(); // key -> {timestamp, event}
    ...
    async startMonitoring(logPath) {
        ...
        // Reset player identification for new session
        if (this.parser) {
            this.parser.resetPlayer();
            this.logger.log('Player identification reset for new session');
        }
    ...
    parseLine(line) {
        try {
            ...
            if (this.parserReady && this.parser) {
                // Check if player was just identified
                const playerBefore = this.parser.getCurrentPlayer();
                const events = this.parser.parseLine(line);
                const playerAfter = this.parser.getCurrentPlayer();

                // If player was just identified, emit a special event
                if (!playerBefore && playerAfter) {
                    this.logger.log(`Player identified: ${playerAfter.name} (ID: ${playerAfter.id})`);
                    this.emit('player-identified', playerAfter);

                    // Also send as an event
                    this.sendEvent({
                        type: 'system',
                        subtype: 'player_identified',
                        name: 'Player Identified',
                        severity: 'low',
                        timestamp: new Date().toISOString(),
                        message: `Player identified: ${playerAfter.name}`,
                        data: playerAfter,
                        raw: line
                    });
                }
     ...
     */
    generateDeduplicationKey(event, pattern) {
        const dedup = pattern.deduplication;

        if (!dedup || !dedup.enabled) {
            return null;
        }

        if (dedup.strategy === 'field-based' && dedup.keys && dedup.keys.length > 0) {
            // Build key from specified fields
            const keyParts = dedup.keys.map(key => {
                return event.data && event.data[key] ? event.data[key] : '';
            });
            return `${event.subtype}:${keyParts.join(':')}`;
        } else if (dedup.strategy === 'timestamp-only') {
            // Just use event subtype
     ...
     */
    shouldDeduplicate(event, pattern) {
        const dedup = pattern.deduplication;
        ...
        const dedupKey = this.generateDeduplicationKey(event, pattern);
        if (!dedupKey) {
            return false;
        }

        const now = Date.now();
        const window = dedup.window || 5000; // Default 5 second window

        // Check if we have a recent matching event
        if (this.eventCache.has(dedupKey)) {
            const cached = this.eventCache.get(dedupKey);
            const timeDiff = now - cached.timestamp;

            if (timeDiff <= window) {
                // Event is a duplicate within the time window
                return true;
            }
        }
        ...
        // Clean up old cache entries (older than window)
        this.cleanEventCache(now, window);

        return false;
    }

    /**
     * Clean up expired entries from event cache
     */
    cleanEventCache(currentTime, maxAge) {
        for (const [key, cached] of this.eventCache.entries()) {
            if (currentTime - cached.timestamp > maxAge * 2) {
    ...
    /**
     * Get pattern configuration for an event
     */
    getPatternForEvent(event) {
        if (!this.parser || !this.parser.patterns) {
            return null;
        }

        // Find the pattern that matches this event's subtype (id)
        const pattern = this.parser.patterns.find(p => p.id === event.subtype);
        return pattern || null;
    }
    ...
    sendEvent(event) {
        ...
        if (pattern && this.shouldDeduplicate(event, pattern)) {
            // Track deduplication stats
            this.deduplicationStats.totalFiltered++;
            if (!this.deduplicationStats.byType[event.subtype]) {
                this.deduplicationStats.byType[event.subtype] = 0;
            }
            this.deduplicationStats.byType[event.subtype]++;

            this.logger.log(`Duplicate event filtered: ${event.type}/${event.subtype} (${this.deduplicationStats.byType[event.subtype]} total filtered)`);
            return;
        }
     ...
     */
    getCurrentPlayer() {
        return this.parser ? this.parser.getCurrentPlayer() : null;
    }
